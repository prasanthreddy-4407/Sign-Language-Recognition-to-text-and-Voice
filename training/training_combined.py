...
# Load alphabet data.
alphabet_data = pd.read_excel(os.path.join(ROOT, "data/alphabet_data (1).xlsx"), header=0)
...
alphabet_groupValue = torch.from_numpy(alphabet_groupValue.to_numpy()).long()

# Load number data.
number_data = pd.read_excel(os.path.join(ROOT, "data/numbers_data (1).xlsx"), header=0)
...
number_groupValue = torch.from_numpy(number_groupValue.to_numpy()).long() + 26  # Shift labels to 26-35

# Combine datasets.
coordinates = torch.cat((alphabet_coordinates, number_coordinates), dim=0)
groupValue = torch.cat((alphabet_groupValue, number_groupValue), dim=0)
...
for fold, (trainIndex, valIndex) in enumerate(kf.split(coordinates)):
    ...
    for epochi in range(1, epoch + 1):
        ...
        valPrec = precision_score(groupValueValidation.cpu(), torch.argmax(outputValid, dim=1), average='weighted',
                                  zero_division=0)
        valRec = precision_score(groupValueValidation.cpu(), torch.argmax(outputValid, dim=1), average='weighted',
                                 zero_division=0)
...
# Save the model.
modelPath = os.path.join(ROOT, "models/CNN_model_combined_SIBI.pth")
